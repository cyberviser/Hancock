...
KB = [
    ...
    {
        "category": "sigma_detection",
        "user": "Write a Sigma rule to detect RDP brute force attacks and successful logins from external IPs.",
        "assistant": (
            "```yaml\n"
            "title: RDP Brute Force Attack\n"
            "id: 5c8d3f7b-2e4a-4b9c-8f1d-7a6e2b5c9d3f\n"
            "status: stable\n"
            "description: |\n"
            "  Detects rapid failed RDP login attempts followed by a successful login,\n"
            "  indicating a successful brute force. Also flags direct RDP access from\n"
            "  external IPs which should require VPN in hardened environments.\n"
            "references:\n"
            ...
            "  service: security\n"
            "detection:\n"
            "  # High-volume logon failures\n"
            "  selection_failure:\n"
            "    EventID: 4625\n"
            "    LogonType: 10       # RemoteInteractive (RDP)\n"
            "  # Successful RDP logon from non-RFC1918 IP\n"
            "  selection_success_external:\n"
            "    EventID: 4624\n"
            "    LogonType: 10\n"
            "    IpAddress|not_cidr:\n"
            "      - 10.0.0.0/8\n"
            ...
            "      - 192.168.0.0/16\n"
            "      - 127.0.0.1/32\n"
            "  condition: selection_failure | count() > 10 over 5m group-by IpAddress\n"
            "            or selection_success_external\n"
            "falsepositives:\n"
            "  - Legitimate remote work from external IPs (should use VPN)\n"
            "  - IT admins connecting from jump hosts\n"
            "  - Load testing tools\n"
            "level: high\n"
            "fields:\n"
            "  - IpAddress\n"
            ...
            "- Disable direct internet-facing RDP (put behind VPN or RD Gateway)\n"
            "- Enable Network Level Authentication (NLA)\n"
            "- Use account lockout policy (threshold ≤5 failures)\n"
            "- Restrict RDP access to specific admin accounts via GPO"
        ),
    },
    {
        "category": "sigma_detection",
        "user": "Write a Sigma rule to detect WMI-based persistence and lateral movement.",
        "assistant": (
            "```yaml\n"
            "title: WMI Event Subscription Persistence\n"
            "id: 7a3c9e2f-5b4d-4f8a-9c1e-6d3b7f2a5e8c\n"
            "status: stable\n"
            "description: |\n"
            "  Detects WMI event subscription creation used for persistence.\n"
            "  Attackers use WMI __EventFilter + __EventConsumer + __FilterToConsumerBinding\n"
            "  to execute payloads on system events (startup, logon, timer).\n"
            "  Used by: APT28, APT29, Turla, FIN6, many commodity malware families.\n"
            "references:\n"
            ...
            "    EventType: 'CreateOrModify'\n"
            "    ObjectNameSpace|contains: 'root\\\\subscription'\n"
            "  selection_binding:\n"
            "    EventType: 'CreateOrModify'\n"
            "    ObjectNameClass: '__FilterToConsumerBinding'\n"
            "  condition: selection_subscription or selection_binding\n"
            "falsepositives:\n"
            "  - Legitimate monitoring tools (System Center, SCOM)\n"
            "  - AV/EDR software using WMI subscriptions\n"
            ...
            "  - EventType\n"
            "```\n\n"
            "**Complementary rule — WMI lateral movement via process creation:**\n"
            "```yaml\n"
            "title: WMI Remote Process Creation (Lateral Movement)\n"
            "id: 9b5e2d7f-3c8a-4f1e-8b4d-2a7c5e9f1b3d\n"
            "logsource:\n"
            "  product: windows\n"
            "  category: process_creation\n"
            "detection:\n"
            ...
            "level: medium\n"
            "```\n\n"
            "**Remediation:** Restrict WMI via `Get-WMIObject -List | Remove-WMIObject` for "
            "unnecessary namespaces. Enable WMI activity logging: `wevtutil sl Microsoft-Windows-WMI-Activity/Operational /e:true`"
        ),
    },
    {
        "category": "sigma_cloud",
        "user": "Write a Sigma rule to detect Azure AD / Entra ID suspicious activity: new MFA method added, role escalation.",
        "assistant": (
            "```yaml\n"
            "title: Azure AD MFA Method Added to User Account\n"
            "id: 4f8b2c5e-7a3d-4e9f-8c2b-5d1a7e4f9c3b\n"
            "status: stable\n"
            "description: |\n"
            "  Detects when an MFA method (authenticator app, phone, FIDO key) is added to an\n"
            "  Azure AD user account. Attackers register new MFA methods after account takeover\n"
            "  to maintain persistent access and bypass MFA challenges.\n"
            "references:\n"
            ...
            "  condition: selection_mfa\n"
            "falsepositives:\n"
            "  - Legitimate users enrolling new MFA devices\n"
            "  - IT helpdesk registering MFA on behalf of users\n"
            ...
            "id: 2e7d5a9f-4b1c-4e8b-9d3f-7c5a2e1b4f8d\n"
            "status: stable\n"
            "description: Detects assignment of privileged Azure AD roles including Global Admin.\n"
            "logsource:\n"
            "  product: azure\n"
            "  service: auditlogs\n"
            "detection:\n"
    ...
    {
        "category": "sigma_detection",
        "user": "Write a Sigma rule to detect DNS tunneling used for C2 communication.",
        "assistant": (
            "```yaml\n"
            "title: DNS Tunneling — High-Volume Subdomain Queries\n"
            "id: 8c4f1a7e-3d9b-4c2f-8e5a-1b7d4c9f2e6a\n"
            "status: stable\n"
            "description: |\n"
            "  Detects DNS tunneling by identifying hosts making an unusually high number of\n"
            "  unique subdomain queries to a single parent domain, which is characteristic of\n"
            "  tools like iodine, dnscat2, DNSExfil, and many RAT C2 channels.\n"
            "  DNS tunneling is also used for data exfiltration (encoding data in query labels).\n"
            "references:\n"
            "  - https://attack.mitre.org/techniques/T1071/004/\n"
            ...
            "  category: dns\n"
            "detection:\n"
            "  selection_long_labels:\n"
            "    # Subdomains > 50 chars are unusual — encoded data often looks like: a3Fb9xQl...domain.com\n"
            "    dns.question.name|re: '^[a-zA-Z0-9+/=]{50,}\\\\..+'\n"
            "  selection_high_entropy:\n"
            "    # High-entropy labels (random-looking): dnscat2, iodine\n"
            "    dns.question.name|re: '^[a-f0-9]{20,}\\\\..+'\n"
            "  condition: selection_long_labels or selection_high_entropy\n"
            "falsepositives:\n"
            "  - CDN providers with long subdomain names\n"
            "  - Azure service bus / AWS endpoints\n"
            "  - Some tracking/analytics domains\n"
            "level: medium\n"
            "fields:\n"
            "  - src_ip\n"
            "  - dns.question.name\n"
            "  - dns.resolved_ip\n"
            "```\n\n"
            "**Volume-based correlation rule (SIEM-side):**\n"
            "```yaml\n"
            "title: DNS Tunneling — Volume Anomaly\n"
            "id: 3e9b7f4d-5a1c-4b8e-9f2d-6c3a5e8b1d7f\n"
            "logsource:\n"
            "  category: dns\n"
            "detection:\n"
            "  timeframe: 1m\n"
            "  condition: count(dns.question.name) by src_ip > 500\n"
            "level: high\n"
            "```\n\n"
            "**Zeek/Bro detection command:**\n"
            "```bash\n"
            "# Identify suspicious DNS clients (>200 unique queries/5min)\n"
            "zeek -r capture.pcap policy/protocols/dns/detect-MHR.zeek\n"
            "```"
        ),
    },
    {
        "category": "sigma_detection",
        "user": "Write a Sigma rule to detect LOLBIN (Living Off the Land Binary) abuse — certutil, regsvr32, rundll32.",
        "assistant": (
            "```yaml\n"
            "title: LOLBIN Abuse — CertUtil Malicious Usage\n"
            "id: 1d5e8b3f-7c2a-4e9b-8d4f-3a6c1e9b5d2f\n"
            "status: stable\n"
            "description: |\n"
            "  Detects suspicious usage of certutil.exe as a LOLBIN for:\n"
            "  - Downloading payloads (-urlcache -split -f)\n"
            "  - Base64 decode (-decode)\n"
            "  - Certificate hash display (intel gathering)\n"
            "  Certutil is a signed Microsoft binary, making it a common AV/proxy bypass.\n"
            "references:\n"
            ...
            "  condition: selection\n"
            "falsepositives:\n"
            "  - PKI admins managing certificates\n"
            "  - Legitimate software installers using certutil\n"
            "level: high\n"
            "---\n"
            "title: LOLBIN Abuse — Regsvr32 Squiblydoo\n"
            "id: 9e2b5f8a-4c7d-4a1e-8b3f-5d2c7a9e4f1b\n"
            "status: stable\n"
            "description: Regsvr32 loading remote SCT files to execute arbitrary code (Squiblydoo).\n"
            "tags:\n"
            "  - attack.defense_evasion\n"
            "  - attack.t1218.010\n"
            "logsource:\n"
